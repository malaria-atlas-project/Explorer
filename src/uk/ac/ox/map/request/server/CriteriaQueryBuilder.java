 ...
 * 
 * Select required after return
 * Only supports chained AND logic in restrictions
 * 
 */
public class CriteriaQueryBuilder {
	
	public static <T> CriteriaQuery<Long> getCountQuery(Class<T> clazz, Map<String, String> sp, CriteriaBuilder criteriaBuilder) {
  	  ...
  	  if (op.equals("contains")) {
  			pred = criteriaBuilder.like(criteriaBuilder.lower(sourceRoot.<String>get(parts[0])), '%' + val.toString().toLowerCase() + '%' );
  	  } else {
   ...
   * @return
   */
  public static <T> CriteriaQuery<T> getQuery(Class<T> clazz, EntityPlace ep, CriteriaBuilder criteriaBuilder) {
  	 ...
  	 * Build search
  	 */
  	Map<String, String> searchParam = PlaceUtils.getMapFromParams(ep.getQueryString());

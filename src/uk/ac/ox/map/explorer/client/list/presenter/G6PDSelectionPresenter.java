 ...
 */
@Singleton
public class G6PDSelectionPresenter extends
		BaseSelectionPresenter<CountryProxy> {

	@Inject
	public G6PDSelectionPresenter(SelectionView selectionView,
			EventBus eventBus) {
	...
	public void start(AcceptsOneWidget panel, EventBus eventBus) {
		...
		eventBus.addHandler(G6PDCheckedEvent.TYPE,
				new G6PDCheckedEvent.Handler() {
					@Override
					public void onCountryChecked(
							G6PDCheckedEvent requestEvent) {
	...
	protected void startDownload() {
		...
		qsb.addParam(RESOURCE_PARAM, G6PD_RESOURCE);

...
import uk.ac.ox.map.explorer.client.rpc.DuffyServiceAsync;
...
public class DuffyDataProvider extends AbstractDataProvider<CountryAllProxy> {

	private DuffyServiceAsync service;
	  private EntityPlace entityPlace;
	  
	  @Inject
	  public DuffyDataProvider(DuffyServiceAsync service,
	      final DuffyTableView view) {
	    super(view);
	    ...
	    service.search(range.getStart(), range.getLength(),
	        entityPlace.getPayload(), new AsyncCallback<ArrayList<CountryAllProxy>>() {
	          ...
	          public void onFailure(Throwable caught) {
	            DuffyDataProvider.this.onFailure(caught);
	          }
